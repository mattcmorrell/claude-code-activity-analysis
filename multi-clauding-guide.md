...
## When to `/clear` and `/compact`

The rule isn't "clear every N turns." It's **clear when the context stops being useful.**

### Don't clear when context is load-bearing

- Building up an analysis where each piece references the last
- Iterating on a single mockup — "move that 10px left," "now change the color" — the model needs the history of what was tried
- Debugging one specific issue — clearing loses the trail of what was already attempted
- Building a tab-switcher mockup with 4 designs — all 4 inform the 5th

If every turn in the conversation is still relevant to what you're doing right now, the context is load-bearing. Don't clear.

### Clear when context becomes dead weight

**Between mockups/explorations:** When mockup A is done and you're starting mockup B, those 30 turns of mockup A iteration (tool outputs, screenshots, dead CSS) are permanent context weight that does nothing for mockup B.

```
Building mockup A → 30 turns of iteration → approved, file saved
→ /clear
Building mockup B → fresh, light context → cheaper turns
```

**After visual debugging:** Screenshots are permanent and incompressible. Once the fix is confirmed, all those "still broken" + screenshot exchanges are dead weight.

```
Turn 1:  "fix this" + screenshot          (+2,500 tokens, permanent)
Turn 5:  "still broken" + screenshot      (+2,500 tokens, permanent)
Turn 9:  "padding is off" + screenshot    (+2,500 tokens, permanent)
Turn 12: "perfect, ship it"
→ /clear ← 7,500 tokens of screenshots you'll never reference again
Turn 13: "next thing..." (fresh context)
```

**When pivoting approach:** If you built a kanban board over 40 turns and decide "actually let's do a timeline view," those 40 turns of kanban code, tool outputs, and error history actively hurt — they're dead weight at best, and at worst they bias the model toward the old approach.

```
"Build me a kanban board" → 40 turns → works but feels wrong
"Actually let's try a timeline view"
→ /clear ← old direction is pure dead weight
```

### Designer/POC workflow cheat sheet

| Scenario | Action | Why |
|---|---|---|
| Finished one HTML exploration, starting another | `/clear` | Old exploration's iterations are dead weight |
| Pivoting design direction ("scrap that, try...") | `/clear` | Old direction's context actively misleads |
| Screenshot-heavy debugging session resolved | `/clear` | Incompressible image tokens accumulated |
| Mid-iteration, context feels heavy but task continues | `/compact` | Shrinks old detail, keeps key decisions |
| Building on prior work (mockup A informs mockup B) | `/compact` | Keeps the relevant decisions, drops verbatim code |
| One long analytical/research thread | Neither | Context is load-bearing, every turn matters |

### `/clear` vs. `/compact`

| | `/clear` | `/compact` |
|---|---|---|
| When | Task is done, starting a new one | Same task, but early detail is stale |
| What it does | Full reset, fresh context | Summarizes old turns, keeps key facts |
| What you lose | Everything (use INTENT.md to preserve) | Verbatim detail from early turns |
| Screenshots | Gone (the point) | **Still there** — compact can't remove images |
| Best for | Between mockups, after pivots, after visual debugging | Mid-session when context is heavy but task continues |

**Important:** `/compact` does not remove screenshots. If your context is bloated because of accumulated images, only `/clear` resets that. This is the main reason to `/clear` after visual debugging rather than just `/compact`.

### The mental model

Think of `/clear` like closing a browser tab when you're done with it. You don't close it mid-task. You close it when that task is done and you're starting something unrelated. The cost of carrying a dead tab is invisible but real — and in Claude Code, it's literally dollars.

Use INTENT.md to preserve cross-session context: what you're building, what's been decided, what was rejected. The new session reads INTENT.md and picks up where you left off, without carrying 200 turns of dead conversation.
...
### 4K / HiDPI displays: the hidden multiplier

Claude auto-resizes images before tokenizing, capping the long edge at ~1568px (Sonnet 4.6 / Opus 4.6) or ~2576px (Opus 4.7). This means full-screen screenshots hit the resize cap regardless of your display resolution — **but crops don't.**

On a 4K display (3840×2160) with macOS Retina scaling, a cropped region that looks like 800×600 on screen is actually **1600×1200 pixels** in the file. This 2x Retina multiplier means every crop from a 4K screen costs ~4x the tokens of the same logical crop from a 1080p screen.

**Full-screen screenshots (hit resize cap):**

| Display | Raw pixels | After resize (Opus 4.6/Sonnet) | Tokens | After resize (Opus 4.7) | Tokens |
|---|---|---|---:|---|---:|
| 1080p | 1920×1080 | 1568×882 | ~1,844 | 1920×1080 | ~2,765 |
| 4K | 3840×2160 | 1568×882 | ~1,844 | 2576×1449 | ~4,977 |

On Opus 4.6/Sonnet, full-screen 4K and 1080p cost the same (~1,844 tokens) — both hit the resize cap. On Opus 4.7, 4K costs **2.7x more** (~4,977 vs ~1,844) because the higher max resolution preserves more detail.

**Cropped screenshots (may NOT hit resize cap):**

| Logical crop size | 1080p pixels | Tokens | 4K Retina pixels | Tokens | 4K cost multiplier |
|---|---|---:|---|---:|---:|
| 400×300 region | 400×300 | ~160 | 800×600 | ~640 | 4x |
| 600×400 region | 600×400 | ~320 | 1200×800 | ~1,280 | 4x |
| 800×600 region | 800×600 | ~640 | 1600×1200 | ~2,560 | 4x |

**This is where 4K really costs you.** A "small" crop on a 4K display is 2,560 tokens — more than a full-screen capture on a 1080p display. If you're pasting 5–10 crops per debugging session, that's 13K–26K tokens of permanent, incompressible context from a 4K screen, vs. 3K–6K from 1080p.

### Strategies to reduce screenshot cost

There's no way for Claude to see rendered output without paying ~1,800+ tokens per look — whether via Playwright, desktop app auto-verify, or manual screenshot paste. The desktop app preview pane and your browser are both free for *your* eyes, but Claude is blind to both unless you send it an image. The savings come from reducing how often Claude needs to look.

**Crop tightly.** On a 4K display, the difference between a generous crop and a tight crop is multiplied by 4x. A crop that's half the width and half the height is 4x fewer tokens — and that savings compounds on every turn for the rest of the session.

**Skip confirmation screenshots.** Only paste a new screenshot when the model needs to see something it can't infer from code. "It's still broken" + screenshot is necessary. "Here's what it looks like now, it works" is pure cost — the model doesn't need visual confirmation.
...
**Describe instead of screenshot when possible.** "The button is misaligned 10px to the left" is ~15 tokens. A screenshot proving it is ~2,500–5,000 tokens on a 4K display. If the description is unambiguous, skip the screenshot.

| Strategy | Token savings (4K display) | Realistic? |
|---|---:|---|
| Crop tightly vs. generous crop | ~1,000–2,000 tokens per screenshot | Yes — already natural |
| Skip confirmation screenshots | ~2,500–5,000 tokens each | Yes — behavioral habit |
| `/clear` after visual debugging | Resets all accumulated screenshots | Yes — one command |
| Describe instead of screenshot | ~2,500–5,000 tokens each | Sometimes — only when unambiguous |

The bottom line for 4K users: **your screenshots cost ~4x more than a colleague on a 1080p display** (for crops) and potentially **2.7x more on Opus 4.7** (for full-screen captures). There's no magic fix — it's a tax on high-resolution displays. The mitigation is behavioral: fewer screenshots, tighter crops, `/clear` after visual work.
...
On screenshots: they're **permanent, incompressible context** — ~1,800–2,560 tokens each (4K crops are 4x more) that can't be compacted away. There's no cheap way for Claude to see rendered output. The savings come from fewer iterations (Opus one-shots more), tighter crops, skipping confirmation screenshots, and `/clear` after visual debugging sessions.
